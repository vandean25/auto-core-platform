...
1. Lock the order’s `WorkshopTask` rows `FOR UPDATE` (sorted ids), then re-validate part-line readiness (no `PENDING_PICK`/`STAGED`, no **active** slice, no tote `quantity_staged`). 409 if any remain.
