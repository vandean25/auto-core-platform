...
8. **Planner overlay is a warning, not 409.** BOOKED leave on a mechanic is amber on `GET /api/workshop/planner` (`employeesAway[]`). Bays can still be booked. Same spirit as mechanic double-book in ADR-0019. **Do not put this warning on the Workshop Board** (ADR-0019: calendar and kanban stay separate). **Do not build `/workshop/planner` in HR** — that page is AUT-177. HR ships the API field, realtime keys, and a reusable amber `Alert` with tests. Mount it on `WorkshopPlannerPage` only if that page already exists on the merge target; otherwise leave the component unmounted for AUT-177.
...
11. **Out of scope:** payroll, overtime, works council, school holidays, OpenHolidays from HR (planner import already exists), merging HR into the kanban, shipping the Workshop Planner calendar page (AUT-177).
...
13. Planner leave warning vs missing planner page — AUT-184 ships `employeesAway` + reusable alert + mechanic punch bar. Do not put the warning on the board. Do not build `/workshop/planner` (AUT-177).
