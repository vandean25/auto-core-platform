...
26. **Requester membership is on either endpoint**, not both. A destination-only user may create a request naming `from_site_id` + qty, and **must not** inspect or set a source bin. Source membership selects `source_location_id` at **approve or ship**. If the requester also has from-site membership, they may suggest a source bin at create. **Create UX:** From-site picker is the names-only directory (`GET /api/sites`), not memberships-only. To-site picker defaults to the current site and also uses the directory so a from-only clerk can name dest. Submit **422** unless the caller has active membership on from **or** to. Source-bin field is omitted unless the caller has from-site membership. **Reads use the same rule** (ruling 43): after approve/ship the line persists `source_location_id`; a dest-only caller opening the shared document must not receive that id or resolved Wien bin details. `from_site_id` / from-site **name** (directory identity) stay visible.
...
42. **Do not merge this PR while the Feature Spec is `draft` or ADR-0022 is `Proposed`.** The commit at the approved head SHA (or the immediate follow-up on that SHA) sets Feature Spec frontmatter `status: approved` and ADR-0022 frontmatter + body plus the ADR index row to **Accepted**. This finding-lock commit keeps `draft` / `Proposed` so the repository does not claim approval before the review does.
43. **Transfer list/detail serializers redact source bins.** Add `GET /api/stock-transfers/:id` (membership on from **or** to; **404** if neither). List and detail omit or null `sourceLocationId` and any resolved source-bin name/code/path unless the caller has **active from-site `SiteMembership` and active `TenantMember`**. `fromSiteId` / from-site directory name remain. Dest-only after approve/ship is the leak path. Realtime transfer payloads apply the same per-recipient redaction (ruling 36).
...
| GET | `/api/stock-transfers` | — | transfers where caller has membership on from **or** to. Source location identifiers/details omitted unless caller has from-site membership (ruling 43) | named cross-site |
| GET | `/api/stock-transfers/:id` | — | full transfer for the detail page. Same membership and source-bin redaction as list. **404** if the caller has neither from nor to membership | named cross-site |
| POST | `/api/stock-transfers` | `fromSiteId`, `toSiteId`, lines (`catalogItemId`, `requestedQty` `> 0`, optional `sourceLocationId` if from-member) | transfer `REQUESTED` | membership on from or to. ≥1 line; omitted/`[]` → 400. Both sites `is_active`. |
...
| `GET /api/customers/:id`, `GET /api/vehicles/:id`, `GET /api/vehicle-stock/:id` | Nested `sales_orders` / `workshop_orders` / `purchases` / `sales` (and their counts) use `listAcrossAuthorizedSites()`, not tenant-only includes (ruling 44). Invoices stay tenant-wide. |
...
- [ ] Wien-only `GET /api/customers/:id` and `GET /api/vehicles/:id` omit München `sales_orders` / `workshop_orders` (and matching counts). Dual-membership caller sees both via the authorized-sites helper. Same for `GET /api/vehicle-stock/:id` nested `purchases` / `sales` / `workshop_orders`.
