...
3. **Invoker IAM.** The OIDC service account must hold `roles/run.invoker` on `core-api-pdf-worker`. The Cloud Run runtime SA that creates tasks must be able to act as the invoker SA (`iam.serviceAccounts.actAs`).
