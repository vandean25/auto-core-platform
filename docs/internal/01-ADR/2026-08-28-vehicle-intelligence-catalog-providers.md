...
- **M3 lock: one `PurchaseOrderItem` per `PartsReservation`.** Linked PO `quantity` is not independently PATCHable (409). Qty changes go through the reservation/line path (DRAFT + unreceived only). DRAFT PO/item delete **cancels** reservations (`CANCELLED`, FK null) so line shortage reappears. `PartsRequisition.status`: terminal states require **at least one slice**. `CANCELLED` iff ≥1 slice and every slice is `CANCELLED` (empty DRAFT is not vacuously cancelled). `COMPLETED` iff ≥1 slice, zero active, and at least one `FULFILLED` (mixed `FULFILLED`+`CANCELLED` with zero active is `COMPLETED`; one `FULFILLED` plus an active slice stays `ORDERED`). `ORDERED` while any slice is `OPEN`/`ORDERED`/`STAGED` after SENT. SENT/received keep the FK (`onDelete: Restrict`). Mark-as-SENT is `OPEN` → `ORDERED` under the same lock order. Qty columns are **`Decimal(10, 3)`** end-to-end (stock, reserved, PO, reservation, workshop/sales lines). `InventoryTransaction.quantity` is already 10,3. Write DTOs (`ReceiveItemDto`, create/update PO item, pick) replace `@IsInt()` with `@IsNumber({ maxDecimalPlaces: 3 })` and min `0.001`. Ledger cache math uses Prisma `Decimal`, not `Number(...)`.
...
| Requisition CANCELLED when no active slices | One idle predicate | All-`FULFILLED` (successful PO) is labelled cancelled |
| COMPLETED = any FULFILLED | Short deletion-policy summary | Completes while another slice is still OPEN/ORDERED/STAGED |
| CANCELLED = ∀ CANCELLED without slice-count ≥ 1 | One predicate | Empty DRAFT is vacuously CANCELLED |
| ORDER BY created_at DESC LIMIT 1 | No schema change | Same-tx receipts share `now()`; UUID `id` is not ordered |
...
| Milestone | M1 Done (AUT-229–AUT-235); M2 JIT (AUT-236–AUT-240); **M3 Requisition, PO & reservations** (AUT-241–AUT-247); M4 wholesaler later |
| Issues | M3: [AUT-241](https://linear.app/auto-core-platform/issue/AUT-241) · [AUT-242](https://linear.app/auto-core-platform/issue/AUT-242) · [AUT-243](https://linear.app/auto-core-platform/issue/AUT-243) · [AUT-244](https://linear.app/auto-core-platform/issue/AUT-244) · [AUT-245](https://linear.app/auto-core-platform/issue/AUT-245) · [AUT-246](https://linear.app/auto-core-platform/issue/AUT-246) · [AUT-247](https://linear.app/auto-core-platform/issue/AUT-247). M2: [AUT-236](https://linear.app/auto-core-platform/issue/AUT-236)–[AUT-240](https://linear.app/auto-core-platform/issue/AUT-240). M1: [AUT-229](https://linear.app/auto-core-platform/issue/AUT-229)–[AUT-235](https://linear.app/auto-core-platform/issue/AUT-235) |
