...
- No `RECEIVED` status: receive + tote transfer is one transaction (`ORDERED` → `STAGED` when complete). Consume auto-sets reservation **`FULFILLED`** when remaining commitment and staged qty are 0. **Active** = remaining commitment > 0 or staged > 0, not `status != CANCELLED`. `FULFILLED` does not block task `DONE`.
...
| STOCK_PREP from CatalogItem.cost_price | One multiply | Historical vehicle cost follows live master |
