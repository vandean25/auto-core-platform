...
- **`StockTransfer`**: Same-GmbH request → approve → ship → receive. Unique `(tenant_id, id)`. Lines copy `from_site_id`/`to_site_id`. List/detail redact `source_location_id` unless the caller has from-site membership. Cross-entity moves are not warehouse transfers.
