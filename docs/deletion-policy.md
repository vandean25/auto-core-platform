...
| Site | Deactivate preferred | Set `is_active = false` **only when** remaining `StockTransfer` rows (from or to) are `COMPLETED`/`REJECTED`/`CANCELLED`; remaining site-owned documents are terminal (`WorkshopOrder` `INVOICED`; `SalesOrder` `INVOICED`; `PurchaseOrder` `COMPLETED`; `VehiclePurchase` `RECEIVED`/`CANCELLED`; `VehicleSale` `INVOICED`/`CANCELLED`); and no on-hand, reserved, or in-transit qty (422 otherwise). There is no `WorkshopOrderStatus.CANCELLED` or `SalesOrderStatus.CANCELLED`. Hard delete only for a pristine unused site: no transfers, no ledger history, no storage locations (except the empty system `in_transit` location, which may be removed **internally** with the site), no memberships, no bays, and no site-owned documents (workshop/sales/purchase/vehicle). Direct delete of the system transit location is forbidden. |
