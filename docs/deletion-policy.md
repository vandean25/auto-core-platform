...
| PartsRequisition | Draft-only | Allow only in `DRAFT`. After `ORDERED`, no hard delete: `COMPLETED` when any slice is `FULFILLED` (including mixed `FULFILLED`+`CANCELLED`); `CANCELLED` only when **every** slice is `CANCELLED`. Do not treat “no active reservations” as cancel. |
