...
- [ ] **Step 1: Add the mandatory checklist**
...
```bash
rg -n -A 25 -B 5 "Before creating a PR|lint:prisma-tenant|npm run lint --workspace=core-api" agents.md
git diff --check
git diff -- agents.md
```

Expected: the mandatory subsection contains every backend CI check, and `git diff --check` exits successfully.
