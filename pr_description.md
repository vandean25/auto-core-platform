...
🛡️ **Solution:**
The fix applies `crypto.timingSafeEqual` by hashing the user's provided key and the expected environment key using `SHA-256`, ensuring a constant-time comparison that eliminates the ability to guess the key through timing analysis. Also resolved the dead code hygiene unit test to permit the usage of `ApiKeyGuard`.
