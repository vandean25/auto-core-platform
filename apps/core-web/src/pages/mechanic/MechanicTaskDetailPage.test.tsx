...
function makeTask(
  overrides: Partial<MechanicTaskDetail> = {},
): MechanicTaskDetail {
...
function createMutationMock(overrides: Record<string, unknown> = {}) {
  return {
    ...
    mutateAsync: vi
      .fn()
      .mockResolvedValue(makeTask({ taskStatus: "IN_PROGRESS" })),
    isPending: false,
    isError: false,
    ...overrides,
  };
...
function setupDefaultMocks(task = makeTask()) {
  ...
  asMock(mechanicApi.usePauseTask).mockReturnValue(
    createMutationMock({
      mutateAsync: vi
        .fn()
        .mockResolvedValue(makeTask({ taskStatus: "WAITING_PARTS" })),
    }),
  );
  ...
  asMock(mechanicApi.useSaveDiagnostics).mockReturnValue(
    createMutationMock({
      mutateAsync: vi
        .fn()
        .mockResolvedValue({ taskId: TASK_ID, mechanicNotes: null }),
    }),
  );
  asMock(mechanicApi.useRequestPart).mockReturnValue(createMutationMock());
  asMock(mechanicApi.useCreateMediaUploadPolicy).mockReturnValue(
    createMutationMock(),
  );
  asMock(mechanicApi.useSaveMediaMetadata).mockReturnValue(
    createMutationMock(),
  );
...
function renderDetailPage(path = `/mechanic/tasks/${TASK_ID}`) {
      ...
      <MemoryRouter initialEntries={[path]}>
        <Routes>
          <Route
            ...
            element={<MechanicTaskDetailPage />}
          />
...
describe("MechanicTaskDetailPage", () => {
  ...
  describe("primary action placement", () => {
    ...
    it('renders "Switch Here" button for NOT_STARTED task', () => {
      ...
      expect(
        screen.getByRole("button", { name: /Switch Here/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("diagnostics auto-save", () => {
    ...
    it('shows "Saving…" indicator while debounce timer is pending', async () => {
      setupDefaultMocks(
        makeTask({ taskStatus: "IN_PROGRESS", mechanicNotes: "" }),
      );
    ...
    it("calls saveDiagnostics after 750ms debounce", async () => {
      const mutateMock = vi
        .fn()
        .mockResolvedValue({ taskId: TASK_ID, mechanicNotes: "Oil dark." });
      asMock(mechanicApi.useSaveDiagnostics).mockReturnValue({
    ...
    it("debounces rapid keystrokes — only the last change triggers save", async () => {
      const mutateMock = vi
        .fn()
        .mockResolvedValue({ taskId: TASK_ID, mechanicNotes: "Final." });
      asMock(mechanicApi.useSaveDiagnostics).mockReturnValue({
        ...createMutationMock({ mutateAsync: mutateMock }),
      });
      asMock(mechanicApi.useMechanicTaskDetail).mockReturnValue({
    ...
    it('shows "Saved ✓" after successful save', async () => {
      const mutateMock = vi
        .fn()
        .mockResolvedValue({ taskId: TASK_ID, mechanicNotes: "Done." });
      asMock(mechanicApi.useSaveDiagnostics).mockReturnValue({
        ...createMutationMock({ mutateAsync: mutateMock }),
      });
      asMock(mechanicApi.useMechanicTaskDetail).mockReturnValue({
  ...
  describe("voice-note recording and draft review", () => {
    it("shows disabled voice-note controls when recording is unsupported", () => {
      ...
      try {
        ...
        const button = screen.getByRole("button", {
          name: /record voice note/i,
        });
        expect(button).toBeDisabled();
        expect(
          screen.getByText(
            /voice note recording is unavailable on this browser/i,
          ),
        ).toBeInTheDocument();
      } finally {
        vi.unstubAllGlobals();
    ...
    it("accepts edited draft into diagnostics notes and keeps autosave path", async () => {
      ...
      const saveMock = vi
        .fn()
        .mockResolvedValue({ taskId: TASK_ID, mechanicNotes: "" });
      const uploadMock = vi.fn().mockResolvedValue({ text: "Initial draft" });

      setupDefaultMocks(
        makeTask({
          taskStatus: "IN_PROGRESS",
          mechanicNotes: "Existing typed note",
        }),
      );
      asMock(mechanicApi.useSaveDiagnostics).mockReturnValue(
      ...
      const diagnosticsTextarea = screen.getByPlaceholderText(
        /record diagnostic findings/i,
      );
      expect(diagnosticsTextarea).toHaveValue(
        "Existing typed note\n\nEdited draft",
      );
    ...
    it("discards draft without changing existing notes", async () => {
      ...
      setupDefaultMocks(
        makeTask({ taskStatus: "IN_PROGRESS", mechanicNotes: "Keep this" }),
      );
      asMock(mechanicApi.useSaveDiagnostics).mockReturnValue(
        createMutationMock({ mutateAsync: saveMock }),
      );
      ...
      expect(
        screen.getByPlaceholderText(/record diagnostic findings/i),
      ).toHaveValue("Keep this");
      expect(
        screen.queryByLabelText(/voice-note draft/i),
      ).not.toBeInTheDocument();
    ...
    it("shows error state when provider upload fails and keeps typed notes", async () => {
      ...
      setupDefaultMocks(
        makeTask({ taskStatus: "IN_PROGRESS", mechanicNotes: "Typed note" }),
      );
      asMock(mechanicApi.useUploadVoiceNote).mockReturnValue(
        createMutationMock({ mutateAsync: uploadMock }),
      );
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("button", { name: /retry recording/i }),
        ).toBeInTheDocument();
      });

      expect(
        screen.getByPlaceholderText(/record diagnostic findings/i),
      ).toHaveValue("Typed note");
  ...
  describe("switch 409 → start fallback", () => {
    ...
    it("falls back to startTask when switch returns 409", async () => {
      const error409 = Object.assign(new Error("No open labor entry"), {
        status: 409,
      });
      ...
      const refetchMock = vi
        .fn()
        .mockResolvedValue({ data: makeTask({ taskStatus: "NOT_STARTED" }) });
    ...
    it("shows success toast after successful start fallback", async () => {
      ...
      await waitFor(() => {
        expect(toast.success).toHaveBeenCalledWith(
          "Task started — punch-in recorded",
        );
      });
    });

    it("shows error toast when both switch and start fallback fail with 409", async () => {
      const error409 = Object.assign(new Error("Already being worked on"), {
        status: 409,
      });
      const switchMock = vi.fn().mockRejectedValue(error409);
  ...
  describe("loading and error states", () => {
    ...
    it("shows not-found message when task is null after load", () => {
      ...
      expect(
        screen.getByText("Task not found or access denied."),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("task content", () => {
    ...
    it("renders odometer reading", () => {
      ...
      const expectedOdometer =
        `${baseTask.odometer.toLocaleString()} km`.replace(/\s+/gu, " ");
