...
describe('Location Hierarchy (e2e)', () => {
    ...
    it('should prevent creating stock in non-BIN location', async () => {
        ...
        // Create Item
        const brand = await prisma.brand.create({ data: { name: 'StockBrand', isPartManufacturer: true } });
        const item = await prisma.catalogItem.create({
            data: { sku: 'SKU-STOCK', name: 'Stock Item', cost_price: 10, retail_price: 20, brand_id: brand.id }
        });
        ...
        const vendor = await prisma.vendor.create({
            data: { name: 'StockVendor', email: 'sv@test.com', account_number: 'SV1', supportedBrands: { connect: { id: brand.id } } }
        });
        ...
        const po = await prisma.purchaseOrder.create({
            data: { 
                ...
                vendor_id: vendor.id, 
                items: { create: [{ catalog_item_id: item.id, quantity: 10, unit_cost: 10 }] } 
            }
        });
