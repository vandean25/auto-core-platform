...
describe('LaborCategoryService', () => {
  ...
  describe('findAll', () => {
    it('returns tree-structured categories with children nested', async () => {
      const childData = {
        id: 'child-1',
        name: 'Sub Engine',
        description: null,
    ...
    it('returns empty data array when no categories exist', async () => {
      mockPrisma.laborCategory.findMany.mockResolvedValue([]);
  ...
  describe('create', () => {
    ...
    it('throws NotFoundException when parent_id does not exist', async () => {
      ...
      await expect(
        service.create({ name: 'Oil Change', parent_id: 'nonexistent' }),
  ...
  describe('update', () => {
    ...
    it('throws NotFoundException when new parent_id does not exist', async () => {
      ...
      await expect(
        service.update('cat-1', { parent_id: 'nonexistent' }),
      ).rejects.toThrow(NotFoundException);
    });
